# Job Manager Harness — visão geral operacional
...
Este documento descreve, em um único lugar, como o harness do Job Manager
...
O Job Manager Harness é a camada de governança local usada para tornar o
...
| [`.claude/skills/`](../../.claude/skills/) | Contém workflows e referências técnicas por boundary | Não é fonte silenciosa de requisitos ou arquitetura do Job Manager |
...
| [`docs/domain/`](../domain/) | Mantém conhecimento de domínio durável quando aceito | Documentos ou decisões `Draft`/`Open` não são normativos |
| [`docs/architecture/`](../architecture/) | Mantém arquitetura transversal e decisões duráveis quando aceitas | Uma feature não pode usá-lo para aceitar silenciosamente uma decisão aberta |
| [`contracts/`](../../contracts/) | Representa contratos externos compartilhados e versionados quando aplicável | A presença de um contrato não autoriza comportamento de produto |
| [`apps/`](../../apps/) | Contém a implementação de backend e frontend | Código existente, testes e exemplos não são requisitos implícitos |
| [`evals/harness/`](../../evals/harness/) | Avalia o comportamento observável do Claude Code sob este harness | Não valida a qualidade funcional da aplicação e não é um benchmark geral do modelo |

## Como o Claude Code encontra essas instruções

O repositório deve ser aberto no Claude Code a partir da pasta que contém o
`CLAUDE.md`, este projeto `job-manager`. A descoberta do `CLAUDE.md` faz parte do
...
A raiz correta também é importante para que `.claude/`, `specs/`,
`docs/`, `contracts/` e `apps/` sejam interpretados como partes do mesmo
repositório. O harness não depende de um caminho absoluto de uma máquina
específica.
...
6. `contracts/` mantém a representação formal compartilhada, quando aplicável;
...
O repositório possui três skills locais:

| Skill | Boundary principal | Cobertura |
| --- | --- | --- |
| [`backend-development`](../../.claude/skills/backend-development/SKILL.md) | `apps/backend/` | Java, Spring Boot, HTTP, JPA, transações, segurança, observabilidade e testes unitários/de integração |
| [`frontend-development`](../../.claude/skills/frontend-development/SKILL.md) | `apps/frontend/` | React, TypeScript, componentes, hooks, formulários, UI states, acessibilidade, browser security, performance, Tailwind, React Router e testes unitários/de integração |
| [`quality-assurance`](../../.claude/skills/quality-assurance/SKILL.md) | jornadas completas no browser | Playwright, E2E, dados e ambiente de teste, diagnóstico de falhas e flakiness |
...
O harness não inventa comandos equivalentes. Ele usa as rotas já definidas:

| Boundary alterada | Validação |
| --- | --- |
| Backend | `apps/backend/mvnw.cmd test` pela convenção real do repositório |
| Frontend | `npm run build` a partir de `apps/frontend` |
| Frontend | `npm run lint` a partir de `apps/frontend` |
| Backend e frontend | combinação dos checks aplicáveis acima |
| Documentação ou harness | inspeções documentais/configuracionais proporcionais; suítes da aplicação não são executadas sem necessidade |
| Jornada completa no browser | workflow Playwright existente, somente quando E2E for realmente aplicável e houver ambiente e serviços necessários |
...
- `contracts/openapi.yaml` é a representação formal compartilhada, quando
  aplicável;
...
No estado atual das decisões registradas, criar `contracts/openapi.yaml` exige a
aceitação prévia de `ARCH-OPEN-008`. Adotar lint, análise de compatibilidade,
publicação, geração de clientes ou stubs exige também a aceitação de
`ARCH-OPEN-004`. Enquanto estiverem `Open`, essas entradas são perguntas, não
...
| [`02 — Ausência de spec`](../../evals/harness/cases/02-missing-spec.md) | Não bloquear apenas pela falta de spec e pausar corretamente diante de ambiguidade material e `DOMAIN-OPEN-007` |
...
- garante sozinho que o Job Manager funciona corretamente;
